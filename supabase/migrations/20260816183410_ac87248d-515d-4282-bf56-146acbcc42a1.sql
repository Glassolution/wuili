...
BEGIN
  ...
  SELECT
    ...
    (CASE WHEN c.v_units > 0 THEN c.v_units ELSE c.ext_sales END)::int AS orders_count,
  ...
  ORDER BY
    ...
    CASE WHEN v_sort = 'demand' THEN c.ext_sales END DESC NULLS LAST,
...
GRANT EXECUTE ON FUNCTION public.get_trending_products(text, text, text, integer, integer) TO authenticated, anon, service_role;